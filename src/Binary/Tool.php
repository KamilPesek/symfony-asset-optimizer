...
{
    /**
     * SHA-256 of every pinned release artifact, keyed by file name (which
     * embeds the version, so bumping {@see version()} fails closed until the
     * new hashes are recorded here). A platform combination without an entry
     * has no artifact upstream and fails before any download.
     */
    ...
    public function sha256(string $os, string $arch): string
    {
        ...
        return self::CHECKSUMS[$file] ?? throw new RuntimeException(sprintf('No release artifact of "%s" for %s/%s ("%s").', $this->value, $os, $arch, $file));
    }
}
